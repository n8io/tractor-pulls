...
const dummyTeamName = 'TeamDeLorean';

const dummyUpstreamOwners = [
  'thefuture',
  'thepast',
  'the-dance',
  'libyan-nationalists',
...
log(`Retrieving team info for ${dummyTeamName}...`, false);
...
request(getOpts(`/orgs/${team}`))
  .then(org => {
    ...
    log(`Retrieving ${dummyTeamName} members...`, false);
  ...
  .then(members => {
    ...
    log(`Retrieving repos for upstream owners [${dummyUpstreamOwners.join(', ')}]...`, false);
  ...
  .then(allRepos => {
    ...
    log(`Retreiving ${dummyTeamName} repos...`, false);
  ...
  .then(upstreamTeamRepos => {
    ...
    return Bluebird
      ...
      .then(upstreamIssueResults => {
        ...
        log(`Retrieving ${dummyTeamName} pull requests...`, false);
  ...
  .then(summary => {
    ...
    const stdOutSummary = obfuscateUsers(getStdOutSummary(summary));
    const slackSummary = obfuscateUsers(slackify(summary));
...
function getStdOutSummary(prSummary, slackifyLinks = false, excludeMentions = EXCLUDE_MENTIONS) {
  ...
  organization.login = dummyTeamName;
  ...
  if (prSummary.upstream && prSummary.upstream.prs.length) {
    ...
    prSummary.upstream.prs
      ...
      .forEach((pr, index) => {
        ...
        const url = slackifyLinks ? `<https://github.com|BackToTheFuture/${dummyServices[index]} #${pr.number}>` : `https://github.com/BackToTheFuture/${dummyServices[index]}/pulls/${pr.number}`;

        upstreamMessages.push(`> ${url}${author} (${relativeCreatedOn})`);
...
function buildPRLink(pr, slackifyUrl = false) {
  if (slackifyUrl) {
    return `<${pr.url}|${dummyTeamName}/${dummyServices[dummyServiceIndex++]} #${pr.number}>`;
  }
  else {
    return `https://github.com.${dummyTeamName}/${dummyServices[dummyServiceIndex++]}/pulls/${pr.number}`;
  }
}
...
function obfuscateUsers(str) {
  if (!str) {
    return str;
  }

  const matches = str.match(/([_~@][a-z]+[~_]?)/g);
  const unames = _.uniq(matches.map(u => u.replace(/[~_@ ]/g, '')));

  dummyUserMap = unames.reduce((map, user, index) => {
    map[user] = dummyUsers[index];
